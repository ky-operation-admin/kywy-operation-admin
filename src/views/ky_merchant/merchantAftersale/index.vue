<template>
  <div class="app-container">
    <div class="title" style="text-align:left;padding:20px">
      <el-input v-model="keyword" @keyup.enter.native="handleFilter" placeholder="请输入分销商家名称/账号" style="width: 300px;margin-right:-10px;" clearable class="filter-item" />
      <el-button class="filter-item" type="primary" icon="el-icon-search" @click="handleFilter">
        查询
      </el-button>
    </div>
    <el-table :data="tableData" style="width: 100%" border>
      <el-table-column prop="merchant_name" label="分销商家名称" align="center"></el-table-column>
        ...
        <template slot-scope="scope">
          ...
          <el-dialog title="编辑" :visible.sync="dialogVisible" width="600px" :before-close="handleClose" :modal-append-to-body="false">
            <el-form ref="form" :model="form" label-width="100px">
              <el-form-item label="分销商家名称" prop="merchant_name">
                <el-input v-model="form.merchant_name" :disabled="true"></el-input>
              </el-form-item>
  ...
  methods: {
    ...
    init () {
      ...
      tempArray = tempArray.reduce(function (item, next) {
        obj[next.merchant_name] ? '' : obj[next.merchant_name] = true && item.push(next);
        return item;
      }, []);
    ...
    handleBlockUp (idx) {
      this.$confirm("确定封停该商家分销资格:" + this.tableData[idx].merchant_name + "?", "提示", {
    ...
    handleRecover (idx) {
      this.$confirm("确定恢复该商家分销资格:" + this.tableData[idx].merchant_name + "?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "success"
      }).then(async () => {
