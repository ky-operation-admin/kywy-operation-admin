    ...
    <el-tabs v-model="activeName">
      ...
      <el-tab-pane label="待审核" name="second" class="havebought">
        <el-table :data="onsaleData0" border v-loading="listLoading">
          ...
          <el-table-column prop="res_name" align="center" label="发布人"></el-table-column>
          <el-table-column prop="serve_name" align="center" label="产品名称"></el-table-column>
          <el-table-column prop="serve_type" align="center" label="产品类目"></el-table-column>
          <el-table-column prop="serve_type" align="center" label="上架数量"></el-table-column>
          <el-table-column prop="serve_type" align="center" label="库存数量"></el-table-column>
          <el-table-column prop="price" align="center" label="价格"></el-table-column>
          <!-- <el-table-column prop="serve_type" align="center" label="产品类型"></el-table-column> -->
          <el-table-column prop="busLicen" align="center" label="产品主图">
            <template slot-scope="scope">
              <img v-image-preview class="busLicen" :src="scope.row.plimg" alt style="height:6rem;" />
            </template>
          </el-table-column>
          <el-table-column prop="busLicen" align="center" label="产品详情图">
            <template slot-scope="scope">
              <img v-image-preview class="busLicen" :src="scope.row.plimg" alt style="height:6rem;" />
            </template>
          </el-table-column>
          <el-table-column prop="busLicen" align="center" label="产品轮播图">
            <template slot-scope="scope">
              <img v-image-preview class="busLicen" :src="scope.row.plimg" alt style="height:6rem;" />
            </template>
          </el-table-column>
          ...
          <el-table-column prop="org_stateText" align="center" label="审核结果" width="170">
            <template slot-scope="scope">
          ...
          <el-table-column align="center" label="操作" >
            <template slot-scope="scope">
              <el-button size="mini" type="primary" @click="ignore(scope.$index)">进入详情页</el-button>
      ...
      <el-tab-pane label="已通过" name="thirdly" class="havebought">
        <div class="title" style="text-align:left;padding:20px">
          <el-input v-model="keyword" placeholder="请输入商家名称/产品ID" style="width: 300px;margin-right:-10px;" clearable class="filter-item" @keyup.enter.native="handleFilter" />
          <el-button class="filter-item" type="primary" icon="el-icon-search" @click="handleFilter">
            查询
          </el-button>
        </div>
        <el-table :data="onsaleData1" border v-loading="listLoading">
          <el-table-column prop="merchant_name" label="商家名称" align='center'></el-table-column>
          <el-table-column prop="org_id" label="产品ID" align='center'></el-table-column>
          <el-table-column prop="res_name" align="center" label="发布人"></el-table-column>
          <el-table-column prop="serve_name" align="center" label="上架产品名称"></el-table-column>
          <el-table-column prop="serve_type" align="center" label="产品类型"></el-table-column>
          <el-table-column prop="busLicen" align="center" label="产品主图">
          ...
          <el-table-column align="center" label="操作">
            <template slot-scope="scope">
              <el-button size="mini" type="primary" @click="unShelve(scope.$index)">下架</el-button>
              <!-- <el-button size="mini" type="danger" @click="blacklist(scope.$index,scope.row)">拉黑</el-button> -->
            </template>
          </el-table-column>
        </el-table>
      ...
      <el-tab-pane label="未通过" name="fourthly" class="havebought">
        <el-table :data="onsaleData2" border>
          ...
          <el-table-column prop="busLicen" align="center" label="产品副图">
            <template slot-scope="scope">
              <img v-image-preview class="busLicen" :src="scope.row.plimg" alt style="height:6rem;" />
            </template>
          </el-table-column>
          ...
          <el-table-column prop="serve_type" align="center" label="未通过理由"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    ignore (idx) {
      ...
      this.$message({
        type: "success",
        message: "审核已被忽略，审核结果已推送给目标商家!"
      });
      this.initData()
    },
    async blacklist (idx, row) {
      this.$confirm("是否确定要拉黑" + row.merchant_name + ", 是否继续?", "提示", {
    ...
    handleFilter () {
      ...
      this.onsaleData1 = this.onsaleData1.filter(item => {
        return item.org_id.includes(this.keyword)
      })
    },
    // 已通过审核商品下架/上架
    unShelve () {
